...
public class MSResizableOreFeature/* extends Feature<MSResizableOreFeatureConfig>*/ {
    ...
    @Override
    public boolean place(@NotNull FeaturePlaceContext<MSResizableOreFeatureConfig> context) {
        RandomSource random = context.random();
        BlockPos pos = context.origin();
        WorldGenLevel world = context.level();
        MSResizableOreFeatureConfig config = context.config();
        ...
        for (int x = minXStart; x <= minXStart + width; ++x) {
            for (int z = minZStart; z <= minZStart + width; ++z) {
                if (minYStart <= world.getHeight(getHeightmapType(), x, z)) {
                    return doPlace(world, random, config, xMin, xMax, zMin, zMax, yMin, yMax,
                            minXStart, minYStart, minZStart, width, height);
                }
            }
        }
        return false;
    ...
    protected boolean doPlace(WorldGenLevel world, RandomSource random, MSResizableOreFeatureConfig config,
                              double xMin, double xMax, double zMin, double zMax, double yMin,
                                                ...
                                                if (world.ensureCanWrite(mutablePos)) {
                                                    ...
                                                    if (section != null) {
                                                        ...
                                                        for (OreConfiguration.TargetBlockState targetState : config.targetStates()) {
                                                            if (canPlaceOre(state, bulkSectionAccess::getBlockState, random,
                                                                    discardChanceOnAirExposure, targetState, mutablePos)) {
                                                                section.setBlockState(sectionX, sectionY, sectionZ,
    ...
    private static boolean canPlaceOre(BlockState state, Function<BlockPos, BlockState> adjacentStateAccessor,
                                       RandomSource random, float discardChanceOnAirExposure,
                                       OreConfiguration.TargetBlockState targetState, BlockPos.MutableBlockPos mutablePos) {

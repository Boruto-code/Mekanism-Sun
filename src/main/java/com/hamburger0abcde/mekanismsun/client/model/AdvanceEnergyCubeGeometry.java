...
public class AdvanceEnergyCubeGeometry implements IUnbakedGeometry<AdvanceEnergyCubeGeometry> {
    ...
    AdvanceEnergyCubeGeometry(List<BlockElement> frame, Map<RelativeSide, List<BlockElement>> leds,
    ...
    @Override
    public @NotNull BakedModel bake(IGeometryBakingContext context, @NotNull ModelBaker baker,
                                    Function<Material, TextureAtlasSprite> spriteGetter, @NotNull ModelState modelState,
        ...
        return new AdvanceEnergyCubeBakedModel(context.useAmbientOcclusion(), context.useBlockLight(),
                context.isGui3d(), context.getTransforms(), overrides, particle, frame, leds, ports, renderTypes);
    }
    ...
    private FaceData bakeElement(Function<String, TextureAtlasSprite> spriteGetter, ModelState modelState, List<BlockElement> elements) {
        ...
        for (BlockElement element : elements) {
            for (Map.Entry<Direction, BlockElementFace> faceEntry : element.faces.entrySet()) {
                ...
                Direction direction = face.cullForDirection() == null ? null :

...
public class RenderAdvanceFluidTank extends MekanismTileEntityRenderer<TileEntityAdvanceFluidTank> {
    ...
    public RenderAdvanceFluidTank(BlockEntityRendererProvider.Context context) {
        super(context);
    }
    ...
    @Override
    protected void render(TileEntityAdvanceFluidTank tile, float partialTick, @NotNull PoseStack matrix, @NotNull MultiBufferSource renderer,
        ...
        if (!fluid.isEmpty() && fluidScale > 0) {
            ...
            MekanismRenderer.renderObject(getFluidModel(fluid, fluidScale), matrix, buffer, MekanismRenderer.getColorARGB(fluid, fluidScale),
                    MekanismRenderer.calculateGlowLight(light, fluid), overlayLight, RenderResizableCuboid.FaceDisplay.FRONT,
        ...
        if (!tile.valveFluid.isEmpty() && !MekanismUtils.lighterThanAirGas(tile.valveFluid)) {
            ...
            MekanismRenderer.renderObject(getValveModel(tile.valveFluid, fluidScale), matrix, buffer,
                    MekanismRenderer.getColorARGB(tile.valveFluid), MekanismRenderer.calculateGlowLight(light, tile.valveFluid),
                            overlayLight, RenderResizableCuboid.FaceDisplay.FRONT, getCamera(), tile.getBlockPos());
        }
    }
    ...
    private MekanismRenderer.Model3D getValveModel(@NotNull FluidStack fluid, float fluidScale) {
        Int2ObjectMap<MekanismRenderer.Model3D> modelMap = cachedValveFluids.computeIfAbsent(fluid, f ->
    ...
    public static MekanismRenderer.Model3D getFluidModel(@NotNull FluidStack fluid, float fluidScale) {
        Int2ObjectMap<MekanismRenderer.Model3D> modelMap = cachedCenterFluids.computeIfAbsent(fluid, fluidStack ->
                new Int2ObjectOpenHashMap<>());

...
public class RenderAdvanceEnergyCube extends ModelTileEntityRenderer<TileEntityAdvanceEnergyCube, ColorModelEnergyCore> {
    public static final Axis coreVec =
            Axis.of(new Vector3f(0.0F, MekanismUtils.ONE_OVER_ROOT_TWO, MekanismUtils.ONE_OVER_ROOT_TWO));

    public RenderAdvanceEnergyCube(BlockEntityRendererProvider.Context context) {
        super(context, ColorModelEnergyCore::new);
    }

    @Override
    protected void render(@NotNull TileEntityAdvanceEnergyCube tile, float partialTick,
        ...
        RenderTickHandler.addTransparentRenderer(new RenderTickHandler.LazyRender() {
            ...
            public void render(Camera camera, VertexConsumer buffer, PoseStack poseStack,
                               int renderTick, float partialTick, ProfilerFiller profiler) {
    ...
    @Override
    public boolean shouldRender(TileEntityAdvanceEnergyCube tile, Vec3 camera) {

...
public class RenderAdvanceMechanicalPipe extends RenderTransmitterBase<TileEntityAdvanceMechanicalPipe> {
    ...
    private static final Int2ObjectMap<Map<FluidStack, Int2ObjectMap<MekanismRenderer.Model3D>>> cachedLiquids =
            new Int2ObjectArrayMap<>(8);

    public RenderAdvanceMechanicalPipe(BlockEntityRendererProvider.Context context) {
        super(context);
    }
    ...
    @Override
    protected void render(TileEntityAdvanceMechanicalPipe tile, float partialTick, @NotNull PoseStack matrix,
                          @NotNull MultiBufferSource renderer, int light, int overlayLight, @NotNull ProfilerFiller profiler) {
        AdvanceMechanicalPipe pipe = tile.getTransmitter();
        ...
        for (Direction side : EnumUtils.DIRECTIONS) {
            ...
            model.setSideRender(side, renderSides[side.ordinal()] ||
                    (side.getAxis().isVertical() && renderBase && stage != stages - 1));
        }
        MekanismRenderer.renderObject(model, matrix, buffer, color, glow, overlayLight,
                RenderResizableCuboid.FaceDisplay.FRONT, camera, tile.getBlockPos());
        if (!connectionContents.isEmpty()) {
            matrix.pushPose();
            matrix.translate(0.5, 0.5, 0.5);
            renderModel(tile, matrix, buffer, MekanismRenderer.getRed(color), MekanismRenderer.getGreen(color),
                    MekanismRenderer.getBlue(color), MekanismRenderer.getAlpha(color), glow, overlayLight,
                    MekanismRenderer.getFluidTexture(fluidStack, MekanismRenderer.FluidTextureType.STILL), connectionContents);
            matrix.popPose();
        }
    }
    ...
    @Override
    protected boolean shouldRenderTransmitter(TileEntityAdvanceMechanicalPipe tile, Vec3 camera) {
    ...
    private MekanismRenderer.Model3D getModel(@Nullable Direction side, FluidStack fluid, int stage, boolean renderBase) {
        ...
        if (model == null) {
            model = new MekanismRenderer.Model3D().setTexture(MekanismRenderer
                    .getFluidTexture(fluid, MekanismRenderer.FluidTextureType.STILL));
    ...
    private static MekanismRenderer.Model3D setHorizontalBounds(Direction horizontal, MekanismRenderer.Model3D.ModelBoundsSetter axisBased,
                                                                MekanismRenderer.Model3D.ModelBoundsSetter directionBased) {

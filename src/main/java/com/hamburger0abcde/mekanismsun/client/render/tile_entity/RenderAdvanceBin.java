...
public class RenderAdvanceBin extends MekanismTileEntityRenderer<TileEntityAdvanceBin> {
    ...
    public RenderAdvanceBin(BlockEntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    protected void render(TileEntityAdvanceBin tile, float partialTick, @NotNull PoseStack matrix, @NotNull MultiBufferSource renderer,
                          int light, int overlayLight, @NotNull ProfilerFiller profiler) {
        Level world = tile.getLevel();
        AdvanceBinInventorySlot binSlot = tile.getBinSlot();
        if (world != null && (!binSlot.isEmpty() || binSlot.isLocked())) {
            ...
            if (blockState.isEmpty() || !blockState.get().canOcclude()
                    || !blockState.get().isFaceSturdy(world, coverPos, facing.getOpposite())) {
                matrix.pushPose();
                ...
                Minecraft.getInstance().getItemRenderer().renderStatic(binSlot.getRenderStack(), ItemDisplayContext.GUI,
                        light, overlayLight, matrix, renderer, world, MathUtils.clampToInt(tile.getBlockPos().asLong()));
                matrix.popPose();
    ...
    protected Component getCount(TileEntityAdvanceBin bin) {
        final AdvanceBinInventorySlot slot = bin.getBinSlot();
    ...
    private void renderText(@NotNull PoseStack matrix, @NotNull MultiBufferSource renderer,
                            int light, int overlayLight, Component text, Direction side, float maxScale) {
        matrix.pushPose();

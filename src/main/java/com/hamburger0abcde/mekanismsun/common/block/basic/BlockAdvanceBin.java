package com.hamburger0abcde.mekanismsun.common.block.basic;
...
public class BlockAdvanceBin extends BlockTile<TileEntityAdvanceBin, BlockTypeTile<TileEntityAdvanceBin>> {
    public BlockAdvanceBin(BlockTypeTile<TileEntityAdvanceBin> type, UnaryOperator<Properties> propertiesModifier) {
    ...
    public void attack(@NotNull BlockState state, @NotNull Level world, @NotNull BlockPos pos, @NotNull Player player) {
        if (!world.isClientSide) {
            TileEntityAdvanceBin bin = WorldUtils.getTileEntity(TileEntityAdvanceBin.class, world, pos);
            if (bin != null) {
                BlockHitResult mop = MekanismUtils.rayTrace(player);
                if (mop.getType() != HitResult.Type.MISS && mop.getDirection() == bin.getDirection()) {
                    AdvanceBinInventorySlot binSlot = bin.getBinSlot();
                    if (!binSlot.isEmpty() && bin.removeTicks == 0) {
                        ...
                        if (!player.getInventory().add(stack)) {
                            ...
                            Entity item = new ItemEntity(world, dropPos.getX() + .5f, dropPos.getY() + .3f,
    ...
    @Override
    @Deprecated
    public ItemInteractionResult useItemOn(@NotNull ItemStack stack, @NotNull BlockState state, @NotNull Level world, @NotNull BlockPos pos,

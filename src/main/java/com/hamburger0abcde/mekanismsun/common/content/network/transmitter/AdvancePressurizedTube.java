...
import com.hamburger0abcde.mekanismsun.common.tiers.transmitter.AdvanceTubeTier;
...
public class AdvancePressurizedTube extends PressurizedTube implements IChemicalTracker,
        IAdvanceUpgradeableTransmitter<PressurizedTubeUpgradeData> {

    public AdvancePressurizedTube(Holder<Block> blockProvider, TileEntityAdvanceTransmitter tile) {
    ...
    private long getAvailablePull() {
        if (hasTransmitterNetwork()) {
            return Math.min(AdvanceTubeTier.getTubePullAmount(tier), getTransmitterNetwork().chemicalTank.getNeeded());
        }
        return Math.min(AdvanceTubeTier.getTubePullAmount(tier), chemicalTank.getNeeded());
    }
    ...
    public long getCapacity() {
        return AdvanceTubeTier.getTubeCapacity(tier);
    }

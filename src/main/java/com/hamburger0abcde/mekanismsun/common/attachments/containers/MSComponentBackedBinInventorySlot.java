...
public class MSComponentBackedBinInventorySlot extends ComponentBackedInventorySlot {
    private final boolean isCreative;

    public static MSComponentBackedBinInventorySlot create(ContainerType<?, ?, ?> ignored, ItemStack attachedTo, int tankIndex) {
        if (!(attachedTo.getItem() instanceof MSItemBlockBin item)) {
            throw new IllegalStateException("Attached to should always be a bin item");
        }
        return new MSComponentBackedBinInventorySlot(attachedTo, tankIndex, item.getAdvancedTier());
    }

    private MSComponentBackedBinInventorySlot(ItemStack attachedTo, int slotIndex, AdvanceBinTier tier) {
        super(attachedTo, slotIndex, ConstantPredicates.alwaysTrueBi(), ConstantPredicates.alwaysTrueBi(), AdvanceBinInventorySlot.validator);
        isCreative = false;
    }

    @Override
    public @NotNull ItemStack insertItem(@NotNull AttachedItems attachedItems, ItemStack current, @NotNull ItemStack stack,
                                         @NotNull Action action, @NotNull AutomationType automationType) {
        if (current.isEmpty()) {
    ...
    /**
     * @see AdvanceBinInventorySlot#getBottomStack()
     */
     ...
     *
     * @see AdvanceBinInventorySlot#setLockStack(ItemStack)
     */

...
public class TileEntityAdvanceInductionPort extends TileEntityAdvanceInductionCasing {
    private final Map<Direction, BlockEnergyCapabilityCache> energyCapabilityCaches = new EnumMap<>(Direction.class);

    public TileEntityAdvanceInductionPort(BlockPos pos, BlockState state) {
        super(MSBlocks.ADVANCE_INDUCTION_PORT, pos, state);
    ...
    public InteractionResult onSneakRightClick(Player player) {
        if (!isRemote()) {
            ...
            setActive(!oldMode);
            player.displayClientMessage(MekanismLang.INDUCTION_PORT_MODE.translateColored(EnumColor.GRAY,

...
public abstract class TileEntityAdvanceLogisticalTransporterBase extends TileEntityAdvanceTransmitter {
    public TileEntityAdvanceLogisticalTransporterBase(Holder<Block> blockProvider, BlockPos pos, BlockState state) {
        super(blockProvider, pos, state);
        addCapabilityResolver(new TileEntityAdvanceLogisticalTransporterBase.TransporterCapabilityResolver());
    }
    ...
    public static void tickClient(Level level, BlockPos pos, BlockState state,
                                  TileEntityAdvanceLogisticalTransporterBase transmitter) {
    ...
    private class TransporterCapabilityResolver implements ICapabilityResolver<@Nullable Direction> {
        ...
        private static final List<BlockCapability<?, @Nullable Direction>> SUPPORTED_CAPABILITY =
                Collections.singletonList(Capabilities.ITEM.block());
        ...
        public <T> T resolve(BlockCapability<T, @Nullable Direction> capability, @Nullable Direction side) {
            ...
            if (cachedCapability == null) {
                ...
                if (transporter.exposesInsertCap(side)) {
                    ...
                    if (cached == null) {
                        cached = new CursedTransporterItemHandler(transporter, WorldUtils.relativePos(getWorldPositionLong(), side),

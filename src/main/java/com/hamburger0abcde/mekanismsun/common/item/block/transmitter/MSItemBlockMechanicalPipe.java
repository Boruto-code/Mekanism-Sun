...
public class MSItemBlockMechanicalPipe extends ItemBlockTooltip<BlockLargeTransmitter<TileEntityAdvanceMechanicalPipe>> {
    ...
    public MSItemBlockMechanicalPipe(BlockLargeTransmitter<TileEntityAdvanceMechanicalPipe> block, Properties properties) {
        ...
        this.color = TierColor.pipeTierToColor(getTier());
    }
    ...
    protected void addStats(@NotNull ItemStack stack, @Nullable TooltipContext context, @NotNull List<Component> tooltip,
        ...
        tooltip.add(MekanismLang.CAPACITY_MB_PER_TICK.translateColored(EnumColor.INDIGO, EnumColor.GRAY,
                TextUtils.format(AdvancePipeTier.getPipeCapacity(tier))));
        tooltip.add(MekanismLang.PUMP_RATE_MB.translateColored(EnumColor.INDIGO, EnumColor.GRAY,
                TextUtils.format(AdvancePipeTier.getPipePullAmount(tier))));
    }
}
